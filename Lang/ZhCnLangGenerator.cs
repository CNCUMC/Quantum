...
public class ZhCnLangGenerator : ModLangGenBase
{
    ...
    {
        // Info
        Option("video.ctrl_to_expand", "Ctrl 更多信息", "按下Ctrl才显示更多信息");
        Option("video.favourited_item_durability_exhaustion_alert", "收藏品耐久警报阈值", "当收藏的物品耐久度低于此比例时发出警报（0 = 关闭）");

        // Item - Gun
        Option("auto_rack", "自动上膛", "如果开启，当有弹药时，枪械将自动拉栓并保持拉栓状态");
        Option("indestructible_gun", "不毁枪械", "如果开启，枪械将不会损坏");
        Option("infinite_ammunition", "无限弹药", "∞ 无限子弹 ∞");
        Option("never_jam", "永不卡壳", "如果开启，枪械将不会卡壳");
        Option("no_casing", "无弹壳", "如果开启，枪械将不会弹出弹壳");
        Option("recoilless", "无后座力", "如果开启，枪械将没有后坐力");

        // Mechanism
        Option("quantum.dont_shit", "不排泄", "如果开启，昏迷时不再排泄");
        Option("quantum.dont_bite_lightbulb", "不咬灯泡", "如果开启，不会再咬灯泡");

        // Misc
        Option("quantum.no_observer", "无观察者", "如果开启，再无观察者");

        // UI
        Option("video.ammunition_ui", "弹药UI", "在原枪械菜单的上方显示枪械剩余弹量和最大弹量");
        Option("video.bilingual_name", "双语名称", "设定后会在物品原名旁附加指定语言的翻译（如 EN / zh-CN / zh-TW），留空则只显示原名");
        Option("input.console_parameter_switching_speed", "控制台参数切换速度", "控制台中长按上下键切换候选参数的速度（毫秒）");
        Option("input.debug_screen", "调试屏幕", "显示调试信息");
        Option("input.debug_screen_fps_graph", "调试屏幕 FPS 图标", "在调试屏幕中显示帧生成时间图表");
        Option("input.hidden_hud", "隐藏 HUD", "隐藏所有 HUD");
        Option("video.debug_screen_speed", "调试屏幕滑动速度", "调试屏幕滑动的速度（毫秒）");
        Option("input.sort_key", "整理按键", "按下整理容器物品");
        Option("video.max_visible_candidates", "最大候选数", "控制台参数候选列表最多显示的行数");
        Option("video.max_history_size", "历史记录上限", "控制台历史命令最多保留的条数");
        Option("video.no_demo_tips", "隐藏试玩提示", "隐藏试玩版提示文字");
        ...
        Other("debug_screen.world.target_block", "目标物块: {0} ({1})");
